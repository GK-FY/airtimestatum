...
client.on('ready', async () => {
  ...
  await alertAdmin('✅ *FY Bot* is online.');
});
...
client.on('message', async msg => {
  try {
    ...
    if(ADMIN_WHATSAPP && fromPhone === ADMIN_WHATSAPP){
      ...
      // /showqr — send last QR as image to admin
      ...
      // /relogin - logout (destroys local session) and reinitialize to force new QR
      if(/^\/relogin$/i.test(body)){
        ...
        // set a short-lived confirmation flag
      ...
      if(/^\/confirmrelogin$/i.test(body)){
        ...
        try {
          ...
          // destroy local auth folder by removing file(s) in SESSION_DIR for this clientId.
          // careful: LocalAuth uses subfolders; easiest approach: logout and then delete LocalAuth files (may vary).
          await client.logout();
          // remove session folder files for a fresh start (best-effort)
          // do not crash if delete fails
          try {
    ...
    // ----- regular user conversational menu (unchanged core) -----
