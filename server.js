...
function updateEnvFile(key, value) {
  try {
    ...
    console.error('updateEnvFile error:', e);
    return false;
  }
}

function getCurrentAdminNumber() {
  // Prioritize settings.json (dashboard changes) over env var
  return (SETTINGS.admin_whatsapp || process.env.ADMIN_WHATSAPP || '').replace(/\D/g, '');
}
...
async function alertAdmin(text){
  const adminNum = getCurrentAdminNumber();
  if(!adminNum) return;
  try{
...
app.get('/admin/settings', adminAuth, (req,res) => {
  const env = {
    admin_whatsapp: getCurrentAdminNumber()
  };
  res.json({ success:true, settings: SETTINGS, env });
});

app.post('/admin/settings', adminAuth, (req,res) => {
  try {
    // Handle admin_whatsapp - save to settings and update .env for persistence
    if (req.body.admin_whatsapp) {
      const newAdmin = String(req.body.admin_whatsapp).replace(/\D/g, '');
      if (newAdmin) {
        SETTINGS.admin_whatsapp = newAdmin;
        // Also update .env file so it persists across restarts
        updateEnvFile('ADMIN_WHATSAPP', newAdmin);
      }
    }
    
    // Save all other settings to settings.json
...
app.get('/admin/system-info', adminAuth, (req,res) => {
  try {
    const info = {
      ...
      whatsappConnected: client.info && client.info.wid ? true : false,
...
client.on('message', async msg => {
  try {
    ...
    // admin WhatsApp special commands (only from configured admin number)
    const currentAdminNumber = getCurrentAdminNumber();
